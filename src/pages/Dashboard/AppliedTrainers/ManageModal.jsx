...
console.log(trainer);
  const handleConfirm = async () => {
    try {
      const {data} = await axiosSecure.patch(`/trainers/${trainer._id}/role`, { role: 'Trainer' });
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div className="relative bg-white p-4 rounded shadow-lg w-full max-w-md">
          <button
            ...
            className="absolute top-2 bg-gray-200 right-2 text-gray-600 hover:text-gray-900 py-1 p-2 text-lg"
          >
            &times;
          </button>
          <h2 className="text-2xl mb-4">Manage Trainer</h2>
          <div className="mb-4">
