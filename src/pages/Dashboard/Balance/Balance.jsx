  ...
  return (
    <div className="w-full mx-auto p-4">
          ...
          <ul className="text-gray-700">
            {transactions.slice(0, 6).map((transaction, index) => (
              <li key={index} className="flex justify-between border-b py-2">
                <span>{index + 1}. {transaction.username}</span>
                <span> {new Date(transaction.orderDate).toLocaleDateString()} {new Date(transaction.orderDate).toLocaleTimeString()}</span>
      ...
      <div className="bg-white shadow-lg rounded-lg p-6 m-4 w-full">
        <h2 className="text-2xl font-bold text-gray-700 mb-2">Subscribers vs Paid Members</h2>
